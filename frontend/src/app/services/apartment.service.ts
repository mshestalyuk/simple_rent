import { Injectable } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';
import { catchError } from 'rxjs/operators';
import { BaseService } from './base.service';
import { ConfigService } from './config.service';

@Injectable({
  providedIn: 'root'
})
export class ApartmentService extends BaseService {
  private get apartmentsUrl(): string {
    return `${this.getBaseUrl()}/simplerent-api/v1/apartments`;
  }

  constructor(
    private http: HttpClient,
    configService: ConfigService
  ) {
    super(configService);
  }

  // Apartment managing methods
  getApartments(): Observable<any[]> {
    return this.http.get<any[]>(this.apartmentsUrl, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }

  addApartment(newApartment: any): Observable<any> {
    return this.http.post(this.apartmentsUrl, newApartment, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }

  deleteApartment(apartmentId: number): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}`;
    return this.http.delete(url, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }

  getApartmentDetails(apartmentId: number): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}`;
    return this.http.get(url, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }

  updateApartment(apartmentId: number, updatedApartment: any): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}`;
    return this.http.put(url, updatedApartment, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }

  // Rent contract methods
  getRentContractDetails(apartmentId: number, contractId: number): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}`;
    return this.http.get(url, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
  
  updateRentContract(apartmentId: number, contractId: number, updatedContract: any): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}`;
    return this.http.put(url, updatedContract, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
  
  deleteRentContract(apartmentId: number, contractId: number): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}`;
    return this.http.delete(url, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
  
  getRentContracts(apartmentId: number): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts`;
    return this.http.get(url, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
  
  addRentContract(apartmentId: number, newContract: any): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts`;
    return this.http.post(url, newContract, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }

  // Tenant methods
  getTenantDetails(apartmentId: number, contractId: number, tenantId: number): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}/tenants/${tenantId}`;
    return this.http.get(url, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
  
  updateTenantDetails(apartmentId: number, contractId: number, tenantId: number, updatedTenant: any): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}/tenants/${tenantId}`;
    return this.http.put(url, updatedTenant, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
  
  deleteTenantDetails(apartmentId: number, contractId: number, tenantId: number): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}/tenants/${tenantId}`;
    return this.http.delete(url, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
  
  getTenants(apartmentId: number, contractId: number): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}/tenants`;
    return this.http.get(url, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
  
  addTenant(apartmentId: number, contractId: number, newTenant: any): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}/tenants`;
    return this.http.post(url, newTenant, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }

  // Document methods
  getDocument(apartmentId: number, contractId: number): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}/document`;
    return this.http.get(url, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
  
  addDocument(apartmentId: number, contractId: number, newDocument: any): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}/document`;
    return this.http.post(url, newDocument, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
  
  deleteDocument(apartmentId: number, contractId: number): Observable<any> {
    const url = `${this.apartmentsUrl}/${apartmentId}/rentcontracts/${contractId}/document`;
    return this.http.delete(url, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError)
    );
  }
}